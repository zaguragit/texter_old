...
class HighlighterSyntaxHighlighter : SyntaxHighlighter() {
    ...
    override fun colorLine(doc: StyledDocument, lineStart: Int, line: String, lineI: Int) {
        if (line.startsWith("//")) {
            doc.setCharacterAttributes(lineStart, line.length, defaultTextStyle().apply { StyleConstants.setForeground(this, Window.theme.gray) }, false)
        } else {
            val tokens = line.split(' ')
            if (tokens.size > 1) {
                var startPos = lineStart
                if (tokens[0].startsWith('@')) {
                    if (tokens[0] in Highlighter.FLAGS) {
